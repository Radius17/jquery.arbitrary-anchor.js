;(function(window, $){
	...
	$(document).ready(function() {
		...
		if(window.AA_CONFIG.scrollOffsetElId.length>0){
			if($('#'+window.AA_CONFIG.scrollOffsetElId).height()){
				window.AA_CONFIG.scrollOffset=window.AA_CONFIG.scrollOffset+$('#'+window.AA_CONFIG.scrollOffsetElId).height();
			}
		}
	...
	function scrollToHash(rawHash) {
		...
		if (($actualAnchor && $actualAnchor.length) || ($actualID && $actualID.length)) return;
